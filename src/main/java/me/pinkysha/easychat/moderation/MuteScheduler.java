...
public final class MuteScheduler {
    ...
    public MuteScheduler(EasyChat plugin, MuteManager muteManager) {
        this.plugin = plugin;
        this.muteManager = muteManager;
    }

    public void scheduleAll(Collection<Mute> mutes) {
        mutes.forEach(m -> schedule(m.playerUuid(), m.expiresAt()));
    }

    public void schedule(UUID uuid) {
        Mute mute = muteManager.get(uuid);
        if (mute != null && mute.expiresAt() != null) {
        ...
        BukkitTask task = plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            ...
            if (mute != null && mute.isExpired(Instant.now())) {
                ...
                muteManager.unmute(uuid).thenAccept(ok -> {
                    if (ok) {
                        plugin.getLogger().info("[UNMUTE] Automatic unmute for "
                                + (playerName == null ? uuid : playerName)
