...
public final class ChatListener implements Listener {
    ...
    public void onChat(AsyncChatEvent event) {
        ...
        if (mute != null && !mute.isExpired(Instant.now())
                && !permissions.canBypassMute(player, plugin.getConfig().getString("moderation.bypass-permission"))) {
            event.setCancelled(true);
            String path = mute.reason().isBlank() ? "moderation.messages.muted-no-reason" : "moderation.messages.muted";
            player.sendMessage(plugin.message(path).replaceText(TextReplacementConfig.builder()
                    .matchLiteral("{reason}")
                    .replacement(mute.reason())
                    .build()));
            return;
        }

        if (plugin.getConfig().getBoolean("chat.cooldown.enabled", true)
                && !permissions.canBypassCooldown(player, plugin.getConfig().getString("chat.cooldown.bypass-permission"))) {
            if (!cooldowns.tryConsume(player.getUniqueId())) {
                event.setCancelled(true);
                long seconds = cooldowns.remainingSeconds(player.getUniqueId());
                player.sendMessage(plugin.message("moderation.messages.cooldown").replaceText(TextReplacementConfig.builder()
                        .matchLiteral("{seconds}")
                        .replacement(String.valueOf(seconds))
                        .build()));
                return;
            }
        }
