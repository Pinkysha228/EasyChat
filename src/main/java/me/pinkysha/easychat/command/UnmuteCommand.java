...
public final class UnmuteCommand implements CommandExecutor, TabCompleter {
    ...
    public UnmuteCommand(EasyChat plugin, MuteScheduler scheduler) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage(plugin.message("messages.usage-unmute"));
            return true;
        }

        Mute mute = plugin.muteManager().getByName(args[0]);
        if (mute == null) {
            sender.sendMessage(plugin.message("moderation.messages.not-found"));
            return true;
        }

        if (sender instanceof Player moderator) {
            if (!plugin.getConfig().getBoolean("moderation.self-unmute", false)
                    && mute.moderatorUuid() != null
                    && mute.moderatorUuid().equals(moderator.getUniqueId())) {
                sender.sendMessage(plugin.message("moderation.messages.cannot-unmute"));
                return true;
            }
        ...
        plugin.muteManager().unmute(mute.playerUuid()).thenAccept(ok ->
                plugin.getServer().getScheduler().runTask(plugin, () -> {
                    ...
                    sender.sendMessage(plugin.replace(
                            plugin.message("moderation.messages.unmuted"),
                            "{player}",
                            playerName
                    ));

                    plugin.getLogger().info("[UNMUTE] " + (sender instanceof Player p ? p.getName() : "Console")
                            + " unmuted " + playerName
                            + " (original moderator=" + mute.moderatorUuid()
                            + ", original weight=" + mute.moderatorWeight() + ")");
                }));
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        plugin.muteManager().getActiveMuteNames().stream()
